...
To assess how faithfully the ray-cast polygon approximates the ground truth mask, the mask IoU is computed between
...
Taken together, Tables~\ref{tab:per-tissue}, \ref{tab:per-type}, and \ref{tab:benchmark-efficiency} provide a direct response to the question of whether contour-based methods warrant reevaluation alongside their mask-based counterparts. In the per-tissue breakdown (Table~\ref{tab:per-tissue}), the mask-based models (CellViT, LKCell) lead the majority of tissue types, confirming that dense mask prediction remains a strong approach for nuclei instance segmentation. However, the contour-based methods (StarDist, CPP-Net) are not far behind: across the 19 tissue types, StarDist and CPP-Net each claim the best mPQ or bPQ on several organs (e.g., CPP-Net wins bPQ on Thyroid and mPQ on Prostate), demonstrating that contour representations are competitive when boundary geometry is regular. The per-type analysis (Table~\ref{tab:per-type}) further reinforces this observation: LSP-DETR, a contour-based model, achieves the highest F1, precision, and recall on four of the five nuclear categories, outperforming the mask-based baselines on per-class detection metrics. Finally, Table~\ref{tab:benchmark-efficiency} confirms the efficiency advantage of the contour paradigm: StarDist and CPP-Net operate at 32--35M parameters and 30--35 GFLOPs, comparable to the mask-based HoVer-NeXt (36M, 24 GFLOPs) but roughly $20\times$ lighter than LKCell (700M, 214 GFLOPs). While their NMS-based post-processing introduces $O(n+k^2)$ complexity, the overall resource footprint remains modest relative to the transformer-class models. These results collectively indicate that contour-based methods, despite being overshadowed in the literature by mask-based and transformer-based approaches, offer a viable balance between detection quality and computational efficiency, and deserve continued investigation.

\subsection{Generalization Test}
\label{sec:generalization}

To evaluate cross-dataset generalization without fine-tuning, the PanNuke-trained RayCastED-B is applied directly to
...
To assess the practicality of RayCastED on resource-constrained hardware, the base variant is tested on two platforms: a desktop GPU (RTX 5070 Ti) and an edge computing device (Jetson Orin Nano). Table~\ref{tab:edge} compares accuracy and throughput across six configurations.

\input{contents/chapter-4/tab/edge}

The PyTorch baseline on the RTX 5070 Ti achieves near-exact predicted nuclei counts, confirming that the end-to-end inference pipeline operates correctly. When compiled with TensorRT, the RTX 5070 Ti reaches a per-image latency of 2.70~ms, representing a substantial speedup over PyTorch. Unexpectedly, TensorRT deployment on the Jetson Orin Nano yields consistently higher bPQ and mPQ than the desktop GPU across both FP16 and FP32 configurations. This is unlikely to stem from reduced precision, since FP32 and FP16 produce near-identical metrics. A plausible explanation is that TensorRT's aggressive graph-level optimizations for the Jetson target architecture restructure the operator execution order in a way that benefits the model numerically. The absence of such gains on the older RTX 2080 Super (similarly running TensorRT) supports the hypothesis that the effect is architecture-specific rather than a universal property of engine compilation.

A qualitative visual check is conducted to compare the output across configurations. Figure~\ref{fig:visual-edge} shows that the predicted nuclear boundaries are visually similar across the desktop PyTorch, desktop TensorRT, and Jetson TensorRT configurations, confirming that the edge deployment preserves detection fidelity despite the substantial differences in hardware and inference backends.
...
To isolate the contribution of key design choices, two controlled experiments are conducted using
