...
This research encompasses five interconnected components, illustrated in Figure~\ref{fig:overall-flow}. First, existing public datasets are processed into a unified annotation format compatible with the raycast representation. Second, a preprocessing pipeline normalizes stains and standardizes image sizes so the model sees a homogeneous input distribution. Third, the RayCastED model is trained, including the architecture itself, the matching assigner, the loss function, and the training curriculum. Fourth, the trained model is exported and deployed to an edge device to evaluate inference feasibility outside. Fifth, a comprehensive evaluation quantifies detection accuracy, segmentation quality, computational efficiency, and cross-dataset generalization.
...
\begin{figure}[H]
    \centering
    \resizebox{0.55\textwidth}{!}{\input{contents/chapter-3/fig/raycast-generation}}
    \caption[RayCast representation of a nuclei]{RayCast representation of a nuclei. From the area-weighted centroid $\mathbf{P}$, rays are cast at uniformly spaced angles. Each distance $d_r$ records the nearest boundary intersection along ray $r$.}
