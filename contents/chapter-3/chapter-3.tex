...
\textbf{Raster Mask to Contour Extraction.}
...
\textbf{Containment Verification and Exterior Centroid Correction.}
...
The algorithm iteratively probes the line segment $[\mathbf{P},\, \mathbf{T}]$ at fractions $\alpha \in \{0.5,\, 0.75,\, 0.875,\, 0.9375,\, 0.96875\}$, testing each candidate $\mathbf{Q}_\alpha = \mathbf{P} + \alpha (\mathbf{T} - \mathbf{P})$ for interiority via the crossing-number test. The candidate is updated only if the test succeeds. After 5 iterations the final precision is $1 - 0.96875 \approx 3.1\%$ of $\|\mathbf{T} - \mathbf{P}\|$. In practice, histopathology nuclei exhibit at most mild concavity, so the area-weighted centroid is interior in nearly all cases and the fallback is rarely triggered.

\textbf{Ray--Edge Intersection.}
...
\textbf{Validity Filtering and Nearest-Hit Selection.}
...
\textbf{Batched GPU Implementation.}
...
\begin{equation}
    \mathbf{V}_x = c_x[:, \text{None}] + \mathbf{D} \odot \mathbf{c}^\top, \qquad
    \mathbf{V}_y = c_y[:, \text{None}] + \mathbf{D} \odot \mathbf{s}^\top
\end{equation}
...
This analytical recomputation decouples centroid accuracy from ray supervision: the ray head is always judged against geometrically correct targets for the position where the model actually placed the centroid, rather than against static targets computed from the ground-truth centroid. If the predicted centroid shifts from the ground-truth centroid, the target rays adapt accordingly, preventing conflicting gradient signals between the centroid and ray branches. Log-space evaluation provides scale-invariant gradients, applying equal relative penalty for over-prediction and under-prediction regardless of absolute ray length.

The centroid loss $\mathcal{L}_{\text{xy}}$ uses Huber loss ($\delta = 0.05$) on normalized grid-cell offsets. The high weight $\lambda_{\text{xy}} = 500$ compensates for gradient attenuation caused by the sigmoid activation, stride multiplication, and pixel-space normalization that the offsets undergo during decoding. The PolarIoU loss $\mathcal{L}_{\text{piou}} = -\log(\text{PolarIoU})$ operates on static ground-truth rays (not analytical) and provides shape-aware geometric supervision through sector-area decomposition, where each sector area is $\frac{1}{2} \sin(2\pi/N) \, d_i \, d_{i+1}$, coupling adjacent rays through shared area computation. The smoothness loss $\mathcal{L}_{\text{smooth}}$ penalizes second-order circular curvature $\left|d_{i+1} - 2d_i + d_{i-1}\right|$ to regularize boundary roughness, with its weight annealed from 0 to 3 as described in the curriculum.
