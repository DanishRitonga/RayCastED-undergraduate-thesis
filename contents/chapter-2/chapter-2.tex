...
This section reviews prior work on nuclei detection and segmentation in histopathology images, focusing on the methods most relevant to the proposed RayCastED model.

\subsection{StarDist}

StarDist \cite{schmidt2018cell} introduced the star-convex polygon representation for cell detection, originally developed for fluorescence microscopy and later adapted to histopathology images \cite{weigert2022stardist}. StarDist predicts a set of radial distances from each cell centroid to its boundary at fixed angular intervals, producing a star-convex polygon that preserves boundary geometry. However, StarDist relies on a two-stage pipeline: it first generates a dense probability map and then applies non-maximum suppression (NMS) to filter overlapping predictions, which risks undercounting in densely clumped regions and introduces computational overhead from post-processing.

\subsection{CPP-Net}

CPP-Net \cite{chen2023cppnet} proposed a context-aware polygon proposal network for nuclei instance segmentation. Rather than regressing radial distances from a fixed grid, CPP-Net generates polygon proposals by learning to predict polygon vertices directly, using a context-aware module that aggregates multi-scale features to refine proposal quality. While CPP-Net demonstrates that polygon-based representations can achieve high segmentation accuracy, it operates within a dense prediction framework and relies on NMS-based post-processing to remove duplicate proposals, inheriting the same NMS dependency as StarDist.

\subsection{HoVer-NeXt}

HoVer-NeXt \cite{baumann2024hovernext} extended the HoVer-Net \cite{graham2019hover} framework for nuclei segmentation and classification, optimizing it for computational efficiency in large-scale histopathology applications. HoVer-Net predicts horizontal and vertical distance maps from each pixel to the centroids of neighboring cells, which are then used to separate touching or overlapping instances through post-processing. HoVer-NeXt improves upon this pipeline with architectural refinements that reduce inference time while maintaining segmentation quality. However, the method remains fundamentally a dense segmentation approach that classifies every pixel and requires computationally expensive post-processing (watershed algorithms and morphological operations) to extract individual instance masks.

\subsection{CellPose}

CellPose \cite{stringer2020cellpose} introduced a generalist framework for cellular segmentation that predicts a gradient flow field pointing from each pixel toward the nearest cell centroid, combined with a cell probability map. Trained on a diverse dataset of over 70,000 segmented objects across multiple image types, CellPose demonstrates strong cross-domain generalization without requiring model retraining. However, CellPose operates as a dense segmentation model that classifies every pixel, making it computationally expensive for whole-slide analysis. A subsequent extension, CellPose-SAM \cite{stringer2025cellposesam}, combined CellPose with the Segment Anything Model (SAM) to achieve superhuman generalization, though the computational profile remains unsuitable for resource-constrained deployment.

\subsection{LSP-DETR}

LSP-DETR \cite{pekar2026lsp} represents the most direct precursor to this work. It introduced a fully end-to-end framework for nuclei segmentation that represents nuclei as star-convex polygons within a DETR-style transformer architecture. By using a learned bipartite assignment mechanism instead of NMS, LSP-DETR eliminates the undercounting problem in clumped regions. The authors also proposed a novel radial distance loss that allows overlapping nuclei segmentation to emerge naturally. LSP-DETR demonstrated strong generalization across tissues and was over five times faster than the next-fastest leading method. However, its transformer-based backbone imposes quadratic complexity, making it computationally demanding for edge deployment. RayCastED is heavily inspired by LSP-DETR's use of star-convex polygon predictions within an end-to-end, NMS-free framework, but is designed from the ground up with a fully convolutional architecture to maintain a lightweight computational profile suitable for resource-constrained environments.

Table~\ref{tab:literature-comparison} summarizes the key characteristics of the reviewed methods, highlighting how each method addresses (or fails to address) the core challenges identified in the background.

\begin{table}[htbp]
	\centering
	\caption{Comparison of prior nuclei detection and segmentation methods. The \textbf{Fully Convolutional} column indicates whether the method uses a pure FCN backbone (no transformer or attention-based components).}
	\label{tab:literature-comparison}
	\resizebox{\textwidth}{!}{%
	\begin{tabular}{lcccl}
		\hline
		\textbf{Method} & \textbf{Approach} & \textbf{End-to-End} & \textbf{Fully Convolutional} & \textbf{Post-processing} \\
		\hline
		StarDist \cite{schmidt2018cell, weigert2022stardist} & Detection & $\times$ & $\checkmark$ & NMS \\
		CPP-Net \cite{chen2023cppnet} & Instance Seg. & $\times$ & $\checkmark$ & NMS \\
		HoVer-NeXt \cite{baumann2024hovernext, graham2019hover} & Instance Seg. & $\times$ & $\checkmark$ & Watershed + Morphological \\
		CellPose \cite{stringer2020cellpose, stringer2025cellposesam} & Instance Seg. & $\times$ & $\checkmark$ & NMS \\
		LSP-DETR \cite{pekar2026lsp} & Detection & $\checkmark$ & $\times$ & None \\
		\hline
	\end{tabular}%
	}
\end{table}

As Table~\ref{tab:literature-comparison} shows, no existing method simultaneously achieves all three properties: end-to-end detection, polygon-based boundary representation, and a fully convolutional architecture. StarDist, CPP-Net, HoVer-NeXt, and CellPose all rely on FCN backbones but require handcrafted post-processing. LSP-DETR is end-to-end through bipartite matching but relies on a transformer backbone. RayCastED is designed to fill this gap by combining the end-to-end bipartite assignment of LSP-DETR with a purely FCN-based architecture.
...
\subsubsection{Area-based Measurements}
...
\subsubsection{Cell Count-based Measurements}

An alternative approach to quantifying immune infiltration is through cell count-based measurements, which estimate the number of immune cells per unit area rather than computing their proportional area. The most prominent example is the \textbf{Immunoscore}, developed and validated by Pag{\`e}s et al. \cite{pages2018immunoscore} for colorectal cancer. The Immunoscore quantifies the density of CD3$^+$ and CD8$^+$ T lymphocytes within two histological regions: the \textbf{tumor core} (CT) and the \textbf{invasive margin} (IM). This yields four density values, expressed as the number of positive cells per square millimeter:

\begin{equation}
	\text{IS} = \bigl\{d_{\text{CD3}}^{\text{CT}},\; d_{\text{CD3}}^{\text{IM}},\; d_{\text{CD8}}^{\text{CT}},\; d_{\text{CD8}}^{\text{IM}}\bigr\}
\end{equation}

where $d_{m}^{r}$ denotes the density of marker $m \in \{\text{CD3}, \text{CD8}\}$ in region $r \in \{\text{CT}, \text{IM}\}$. Each density value is classified as high (1) or low (0) based on a predefined percentile threshold determined from a reference patient cohort. The four binary values are then summed to produce the final Immunoscore:

\begin{equation}
	\text{Immunoscore} = \sum_{m \in \{\text{CD3}, \text{CD8}\}} \sum_{r \in \{\text{CT}, \text{IM}\}} \mathbb{1}\bigl[d_{m}^{r} \geq \tau_{m}^{r}\bigr]
\end{equation}

where $\tau_{m}^{r}$ is the density threshold for marker $m$ in region $r$ and $\mathbb{1}[\cdot]$ is the indicator function. The resulting score ranges from I0 (all four densities low) to I4 (all four densities high), with higher scores indicating stronger immune infiltration and consistently associated with improved patient survival across multiple validation cohorts.

Unlike area-based TILs scoring, count-based measurements do not require accurate boundary delineation of each individual cell. A detection model that correctly identifies cell centroids is sufficient to count cells per unit area. However, count-based methods typically require immunohistochemical (IHC) staining for specific markers (e.g., CD3, CD8) rather than standard H\&E staining, which limits their applicability in resource-constrained settings where IHC is not routinely available. Furthermore, the area-based sTILs methodology has been more widely adopted across cancer types, particularly in breast cancer, making it the more broadly relevant measurement for automated analysis systems.
...
Semantic segmentation assigns a class label to every pixel in an image, producing a dense classification map where each pixel is labeled as belonging to one of several classes. Formally, given an input image $\mathbf{X} \in \mathbb{R}^{H \times W \times C}$, a semantic segmentation model produces a label map $\mathbf{Y} \in \{1, \ldots, K\}^{H \times W}$ where $K$ is the number of classes and each $Y_{i,j}$ indicates the class of pixel $(i,j)$.

The standard architecture for semantic segmentation is the fully convolutional network (FCN), which replaces the fully connected layers of a classification network with convolutional layers to produce spatially dense outputs. The encoder-decoder structure is common: an encoder backbone (e.g., ResNet, VGG) progressively downsamples the input to extract high-level features, while a decoder progressively upsamples these features to recover the original spatial resolution. Skip connections between corresponding encoder and decoder layers help preserve fine-grained spatial details that would otherwise be lost during downsampling.

Semantic segmentation is effective for macro-level area measurements, such as estimating the overall proportion of stromal tissue occupied by lymphocytes. However, it does not distinguish between individual instances of the same class: all nuclei receive the same label regardless of their spatial separation, making it impossible to count individual cells.

\subsubsection{Instance Segmentation}

Instance segmentation extends semantic segmentation by assigning a unique label to each individual object instance, producing both a class label and an instance identity for every pixel. Given an input image, the output is a label map where each pixel is assigned a pair $(c, k)$ indicating that it belongs to instance $k$ of class $c$.

Two dominant paradigms exist for instance segmentation. \textbf{Proposal-based methods} such as Mask R-CNN \cite{he2017mask} operate in two stages: a region proposal network (RPN) generates candidate object bounding boxes, and a mask head then predicts a binary segmentation mask for each proposal. While accurate, the two-stage pipeline introduces significant computational overhead, particularly when the number of instances is large.
...
Segmentation-based approaches face two fundamental limitations when applied to nuclei analysis in histopathology images.
...
Object detection models operate by dividing the input image into a spatial grid and predicting a fixed set of parameters (typically a bounding box and a class label) at each spatial location. Unlike segmentation, which classifies every pixel, detection produces a sparse set of structured outputs that describe the position and extent of individual objects. This grid-based prediction framework forms the foundation of modern single-stage detectors such as YOLO \cite{redmon2016yolo} and two-stage detectors such as Faster R-CNN \cite{ren2015faster}, and is the paradigm adopted in this work.

\subsubsection{Anchor Grid}
...
Detection models offer computational advantages over segmentation but face two critical shortcomings when applied to nuclei analysis.

The \textbf{boundary problem} is that bounding box representations discard cell morphology. A bounding box encloses the approximate spatial extent of a cell but does not capture its shape. For approximately circular nuclei, the difference between bounding box area and true cell area may be modest, but for irregularly shaped or overlapping nuclei, bounding boxes significantly overestimate cell size and destroy the geometric information necessary for area-based measurements such as TILs scoring. A detection model can identify where cells are but cannot determine how much area each cell occupies.

The \textbf{density problem} compounds the boundary problem in the very regions where accuracy matters most. In densely clumped areas, cells are closely spaced and their bounding boxes overlap heavily. NMS must then decide which overlapping boxes correspond to distinct cells and which are duplicate predictions of the same cell. Since this decision is based solely on geometric overlap and confidence scores, NMS systematically undercounts in clumped regions: valid detections of adjacent cells are suppressed because their boxes overlap, while in sparse regions where cells are well-separated, NMS has little effect. This creates a density-dependent counting bias where the model performs worst precisely in the regions most relevant to clinical assessment.
...
This bipartite matching formulation is the foundation of end-to-end detection frameworks such as DETR and LSP-DETR \cite{pekar2026lsp}. By guaranteeing a unique assignment, it eliminates the need for NMS as a post-processing step, since each ground-truth object is matched to exactly one prediction and there are no duplicate assignments to suppress.
...
\begin{itemize}
	...
	\item \textbf{Masked PQ ($\text{MPQ}$):} a variant introduced by LSP-DETR \cite{pekar2026lsp} designed for fair evaluation of overlapping nuclei. Standard PQ penalizes polygon predictions that cross artificially separated ground-truth boundaries in regions of biological overlap. MPQ replaces the standard IoU with a \textbf{masked IoU (mIoU)} that isolates evaluation to the matched ground-truth instance. Given a ground-truth mask $m_k$ and predicted polygon $\hat{m}_k$, and letting $\mathcal{G} = \bigcup_{k=1}^{K} m_k$ be the union of all ground-truth masks:

	\begin{equation}
		\text{mIoU}(m_k, \hat{m}_k) = \frac{|m_k \cap \hat{m}_k|}{|m_k \cup (\hat{m}_k \setminus \mathcal{G})|}
	\end{equation}

	\begin{equation}
		\text{MPQ} = \underbrace{\frac{|TP|}{|TP| + \frac{1}{2}|FP| + \frac{1}{2}|FN|}}_{\text{RQ}} \times \underbrace{\frac{\sum_{(m_k,\hat{m}_k) \in TP} \text{mIoU}(m_k, \hat{m}_k)}{|TP|}}_{\text{SQ}_{\text{mask}}}
	\end{equation}

	The denominator in mIoU includes the ground-truth area plus only those parts of the prediction that fall outside $\mathcal{G}$, meaning predictions overlapping \textit{other} ground-truth masks beyond the matched one are penalized, but overlap with the matched instance itself is not double-counted. This makes MPQ the appropriate metric for polygon-based methods that output realistically overlapping cell representations.
\end{itemize}
