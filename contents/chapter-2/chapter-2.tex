...
\subsubsubsection{Stochastic Gradient Descent with Momentum}
...
\subsubsubsection{Muon Optimizer}
\label{sec:muon}

The Muon optimizer \cite{chen2025muon} is a recent optimization algorithm designed for training neural networks with matrix-structured parameters. Unlike standard optimizers such as SGD or AdamW that treat each parameter independently, Muon exploits the matrix structure of weight tensors by applying momentum accumulation followed by Newton-Schulz orthogonalization to constrain the spectral norm of the weight updates.

For a weight matrix $W \in \mathbb{R}^{m \times n}$ with gradient $G_t = \nabla_W \mathcal{L}(W_t)$, Muon first computes a momentum term analogous to SGD with momentum:

\begin{equation}
    M_t = \mu M_{t-1} + G_t
\end{equation}

where $\mu$ is the momentum decay rate. The momentum $M_t$ is then orthogonalized through $k$ iterations of the Newton-Schulz algorithm, which approximates the polar factor $\widetilde{M}_t = UV^\top$ where $M_t = U \Sigma V^\top$ is the singular value decomposition. Starting from $X_0 = M_t / \|M_t\|_F$, each iteration computes:

\begin{equation}
    X_{i+1} = \frac{3}{2} X_i - \frac{1}{2} X_i X_i^\top X_i
\end{equation}

After $k$ iterations (typically $k = 5$), the orthogonalized update $\widetilde{M}_t \approx X_k$ is obtained. Empirically, $k = 5$ Newton-Schulz steps suffice to achieve a near-orthogonal matrix, avoiding the $O(\min(m,n)^3)$ cost of an exact SVD. The weight is then updated as:

\begin{equation}
    W_{t+1} = W_t - \eta \cdot \widetilde{M}_t
\end{equation}

where $\eta$ is the learning rate. The Newton-Schulz iteration converges to the polar factor, which implicitly constrains the spectral norm $\|W_t\|_2$ during training. This spectral normalization has been shown to improve training stability and generalization by preventing individual weight matrices from developing disproportionately large singular values that would dominate the parameter norm \cite{chen2025muon}. Muon has demonstrated strong empirical performance in training vision and language models, achieving faster convergence than AdamW while maintaining comparable or better final accuracy.

\subsubsection{Assignment}
\label{sec:assignment}

\subsubsubsection{Task-Aligned Matching}
\label{sec:tal}
...
\subsubsubsection{Dual-Label Assignment}
