\begin{longtable}{l c c l >{\footnotesize}p{0.28\textwidth}}
	...
	\endlastfoot
		...
		NuLite \cite{tommasino2024nulite}
			& 2024 & Tommasino et al.
			& mask-based
			& U-Net-like architecture built on Fast-ViT lightweight backbone; three scaled variants (S/M/H) trade capacity for speed; matches CellViT accuracy with up to $40\times$ fewer parameters; watershed post-processing \\
		FKDNuSeg \cite{zhao2026fkdnuseg}
			& 2026 & Zhao et al.
			& mask-based
			& Flawless knowledge distillation from a heavy teacher to a lightweight student segmentation network; joint instance segmentation and classification with distillation on both pixel-level and instance-level outputs; watershed post-processing \\
		LSP-DETR \cite{pekar2026lspdetr}
			& 2026 & Pekár et al.
			& contour-based
			& Lightweight linear-complexity transformer processes larger image patches without added cost; nuclei represented as star-convex polygons; radial distance loss enables overlapping nuclei segmentation without explicit overlap annotations or handcrafted post-processing; fully end-to-end, NMS-free \\
		\hline
\end{longtable}
