...
\begin{frame}{Clinical Background: Immune Cell Density Scoring}
  \begin{columns}
    \column{0.6\textwidth}
    \begin{itemize}
      \item \textbf{Immune Cell Density} quantifies the absolute number of immune cells within a defined tissue region.
      \item Represents a shift from traditional visual area estimation to precise \textbf{single-cell enumeration}.
      \item \textbf{Critical requirement}: robust \textbf{instance segmentation} to accurately detect, separate, and classify individual nuclei.
    \end{itemize}

    \column{0.4\textwidth}
    \begin{alertblock}{The Challenge}
      Manual single-cell counting is:
      \begin{itemize}
        \item Practically impossible at scale
        \item Highly prone to human error
        \item Requires digital AI pipelines for whole-slide analysis
      \end{itemize}
    \end{alertblock}
  \end{columns}
  ...
  \begin{equation*}
      \text{Immune Cell Density} = \frac{\text{Total count of detected immune cells}}{\text{Analyzed tissue area (mm}^2\text{)}}
  \end{equation*}
\end{frame}
...
\begin{frame}{The Clumped Cell Problem}
  \begin{columns}
    ...
    \begin{itemize}
      ...
      \item \textbf{Result}: Cell undercounting
      \item Existing solutions require costly post-processing
      \item Non-Max Suppression (NMS) suppresses overlapping predictions (risky in clumped regions)
    \end{itemize}
...
\begin{frame}{Research Workflow}
  \begin{center}
    \resizebox{0.75\textwidth}{!}{\input{../contents/chapter-3/fig/research-workflow.tex}}
  \end{center}
...
\begin{frame}{Data Preprocessing Pipeline}
  \begin{center}
    \resizebox{0.95\textwidth}{!}{\input{../contents/chapter-3/fig/data-preprocessing.tex}}
  \end{center}
...
\begin{frame}{Data Ingestion Design}
  \begin{center}
    \resizebox{0.75\textwidth}{!}{\input{../contents/chapter-3/fig/ingestor-uml.tex}}
  \end{center}
 
  \begin{itemize}
    \item Multi-format dataset handling (TIFF, PNG, pickled bytes)
    \item Object-oriented design for modularity and extensibility
  \end{itemize}
\end{frame}
...
\begin{frame}{Key Innovations}
  \begin{columns}
    ...
    \begin{block}{1. RayCast Representation}
      \begin{itemize}
        \item Centroid $(x, y)$ + $N$ radial distances
        \item Lightweight, boundary-aware representation
      \end{itemize}
    \end{block}
  ...
  \vspace{1em}
  \begin{block}{3. Lightweight FCN Backbone}
    \begin{itemize}
      \item Efficient wavelet-enhanced feature extraction
      \item TensorRT/ONNX compatible
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Evaluation Procedure}
  \begin{center}
    \resizebox{0.9\textwidth}{!}{\input{../contents/chapter-3/fig/evaluation.tex}}
  \end{center}
\end{frame}

\begin{frame}{Evaluation Against LSP-DETR}
\input{../contents/chapter-4/tab/eval}
